...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else if (detail && typeof detail === "object" && "message" in detail) setError(detail.message || "Signup failed");
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-8" style={{ background: "var(--bg-primary)" }}>
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-xl p-6" style={{ background: "var(--bg-secondary)", border: "1px solid var(--border-color)" }}>
          <h2 className="text-xl font-semibold mb-1" style={{ color: "var(--text-primary)", letterSpacing: "-0.02em" }}>Create account</h2>
          <p className="text-[13px] mb-6" style={{ color: "var(--text-muted)" }}>Start creating AI-powered ads</p>
